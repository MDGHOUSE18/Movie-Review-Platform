...
app.post("/usermovie", (req, res) => {
  ...
  const url = req.body.url;
  db.execute(
    "select * from usermovies where name=? and url=?",
    [moviename, url],
    (err, result) => {
      if (err) throw err;
      if (result.length > 0) {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result.length < 1) {
        ...
        db.execute(
          "update review set value=?, rating=? where user_id=? and movie_id=? ",
...
app.post("/rating", (req, res) => {
  ...
  db.query(
    "SELECT value FROM review WHERE user_id = ? AND movie_id =?",
    [userid, movieid],
    (err, results, fields) => {
      if (err) throw err;
      if (results.length > 0) {
...
app.post("/delete", (req, res) => {
  ...
  const userid = req.body.user_id;
  db.query(
    "delete from review where movie_id=? and user_id=?",
    [movieid, userid],
    (err, results) => {
      if (err) throw err;
      res.send({ message: "success" });
...
app.post("/moviedelete", (req, response, fields) => {
  ...
  db.query(
    "delete from usermovies where name=? and sl=?",
    [name, id],
    (err, results) => {
      if (err) throw err;
