  ...
  if (session == "null") {
    return (
      <>
        ...
        <Link to="/Login">
          {" "}
          <button class="mybtn3">Login</button>
        </Link>
        <span class="text-center"> or</span>
        <Link to="/Signup">
          {" "}
          <button class="mybtn3">Signup</button>
        </Link>
        <Outlet />
      </>
    );
...
export default function Movies() {
  ...
  const getData = () => {
    fetch("http://localhost:3001/Myrating", {
      ...
      .then(function (myJson) {
        ...
        const movieNames = [];
        myJson.map((movie) => {
          if (movie.user_id == session && movie.movie_id == location.state.id) {
            movieNames.push(movie.value);
          }
        });
  ...
  {
    data.map((item2) => {
      if (location.state.id == item2.movie_id) {
          ...
          <div class="container">
            ...
            <div class="content">
              <div class="row">
                <div class="col-md-6">
                  <figure class="movie-poster">
                    <img class="imgsize" src={location.state.url} alt="#" />
                  </figure>
                </div>
                <div class="col-md-6">
                  ...
                  <ul class="movie-meta">
                    <li>
                      <strMessageong>
                        Rating: {x / count} out of 10{" "}
                      </strMessageong>
                    </li>
                    <li>
                      <strMessageong>Release:</strMessageong>{" "}
                      {location.state.year}
                    </li>
                    <li>
                      <strMessageong>Langauge:</strMessageong>{" "}
                      {location.state.language}
                    </li>
                    <li>
                      <strMessageong>Category:</strMessageong>{" "}
              ...
              <div class="row">
                ...
                <hr></hr>
                {data.map((item) => {
                  if (item.movie_id == location.state.id) {
                    return (
                      <div class="col-lg-3">
                        <div class="row">
                          <div class=" review-box ">
                            ...
                            <div class="col-lg-9">
                              <span>{item.rating}/10</span> <p>{item.value}</p>
                            </div>
                          </div>
                        </div>
                      </div>
